...
from keras.layers import Conv2D, Cropping2D, Dense, Dropout, Flatten, Lambda, MaxPooling2D, concatenate, Input
...
def get_fusion_model(img_shape):
    left_model = get_model()
    right_model = get_model()
    
    left_camera = Input(shape=( 
    
    main = concatenate([left_camera, right_camera])
...
def main():
    ...
    model = get_fusion_model(img_shape)
    print('[#main]: training the model')
    model.compile(optimizer='adam', loss='mse')
    model.fit([left_data, right_data], [labels, labels], batch_size=100, epochs=7, validation_split=.2)

    tokens = (args.bag or args.numpy_data).split('/')
    bag_name = tokens[-1]
    print('[#main]: saving to {}.h5'.format(args.save_dir + bag_name))
    model.save('{}.h5'.format(args.save_dir + bag_name))
