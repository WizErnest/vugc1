...
def callback(message):
    print("[#callback]: received message", message.img.header.stamp)
    try:
        image = bridge.imgmsg_to_cv2(message.img)[:,:,0:3]
        start = time.time()
...
def main():
    try:
        ...
        s = rospy.Service('vugc1_perception_detection', Detection, callback)
